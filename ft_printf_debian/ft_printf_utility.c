...
void	print_cleaning(char c, t_flag *bonus, int zero)
{
	write(1, "%", 1);
	if (bonus->prefix)
		write(1, "#", 1);
	if (bonus->plus_space)
		print_plus_minus(1, bonus);
	if (bonus->padding_right)
		print_plus_minus(-1, bonus);
	if (!bonus->padding_right && zero)
	{
		write(1, "0", 1);
		bonus->printed_char++;
	}
	if (bonus->width)
		ft_putjustnbr(bonus->width, 1, bonus);
	if (bonus->dot)
	{
		write(1, ".", 1);
		ft_putjustnbr(bonus->precision, 1, bonus);
	}
	write(1, &c, 1);
	bonus->printed_char += bonus->prefix + bonus->precision + 2;
}
